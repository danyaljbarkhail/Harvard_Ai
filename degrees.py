...
def shortest_path(source, target):
    ...
    # Initialise frontier to the source
    ...
    while True:
        ...
        # Choose a node from the frontier and add to explored set
        ...
        if node.state == target:
            ...
            while node.parent is not None:
                connection = (node.action, node.state)
        ...
        for movie, actor in neighbors_for_person(node.state):
            if not frontier.contains_state(actor) and actor not in explored:
                new_neighbour = Node(state=actor, parent=node, action=movie)
                frontier.add(new_neighbour)
